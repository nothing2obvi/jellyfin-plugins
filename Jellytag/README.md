...
After a burst of normal browsing image requests goes quiet, JellyTag-Plus also performs a small recent request completion check. It remembers recent uncached overlay renders, waits briefly, checks whether they reached cache, and retries missing ones once at low impact. This is intentionally conservative and may be reverted if it proves unhelpful.
...
JellyTag-Plus `1.51.20.0` is the final Jellyfin 10.11-supported release. That package targets Jellyfin ABI `10.11.0.0` and .NET 9.
...
Jellyfin only auto-updates plugins when the repository package version is higher than the installed version. Because Jellyfin 12+ starts at `1.52.0.0`, a server upgraded from Jellyfin 10.11 can replace the final `1.51.20.0` package on the next plugin update.
